...
def get_image_cards(image,
    ...
    if not max_boxes_to_draw:
        max_boxes_to_draw = boxes.shape[0]
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        #if there are no scores or the scores val is greater than max
        if scores is None or scores[i] > min_score_thresh:
            cards_in_image.append([])
            if scores is None:
                scores[i]=-1
            else:		
		#print("classes",classes[i], "keys", category_index.keys()) # AMW
                if classes[i] in category_index.keys():
                    class_name = category_index[classes[i]]['name']
                    cards_in_image[i].append(class_name)
                    cards_in_image[i].append(scores[i])
                else:
                    class_name = 'N/A'
                    cards_in_image[i].append(class_name)
                    cards_in_image[i].append(scores[i])
